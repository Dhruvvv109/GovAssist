...
// ── Check if any reminder is due today ────────────────────────────────────
...
function DaysBadge({ days }) {
  ...
  if (days === 0) return <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-red-600 text-white animate-pulse">Today!</span>
...
export default function Deadlines() {
  ...
  // Check today's reminders on mount
  useEffect(() => {
  ...
  // Dates that have deadlines — for calendar tile marking
  const deadlineDates = schemes.map(s => s.deadline)

  const tileContent = ({ date, view }) => {
    if (view !== 'month') return null
    const iso = date.toISOString().slice(0, 10)
    const count = deadlineDates.filter(d => d === iso).length
  ...
  const tileClassName = ({ date, view }) => {
    ...
    const iso = date.toISOString().slice(0, 10)
    if (deadlineDates.includes(iso)) return 'has-deadline'
    return ''
  }

  // Schemes on selected calendar date
  const selectedIso = calDate.toISOString().slice(0, 10)
  const selectedSchemes = schemes.filter(s => s.deadline === selectedIso)

  // Upcoming = not expired, sorted by daysLeft
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
      ...
      {/* Top grid: Calendar + Selected day */}
      <div className="grid lg:grid-cols-2 gap-6 mb-8">

        {/* Calendar */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <p className="text-sm font-semibold text-gray-700 mb-4">Click a date to see deadlines</p>
          <style>{`
            ...
            .react-calendar__tile--now { background: #eef2ff; }
          ...
          {selectedSchemes.length === 0 ? (
            ...
            <div className="space-y-3">
              {selectedSchemes.map(s => (
                <div key={s.id} className="flex items-start justify-between gap-3 p-3 bg-primary-50 rounded-xl border border-primary-100">
                  <div>
